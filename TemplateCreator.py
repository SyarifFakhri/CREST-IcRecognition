...
while True:
    ...
    imgResized = cv2.resize(img, (ICRecognition.widthImg, ICRecognition.heightImg), interpolation=cv2.INTER_LINEAR)
    cv2.imshow("resized", imgResized)

    histogram = ICRecognition.calcHistogram(imgResized)
    histogram = cv2.GaussianBlur(histogram, (21, 21), 0)
    hist = ICRecognition.drawHistogram(histogram, 256, 200)
    ...
    largestContourInImage = ICRecognition.returnLargestAreaOfContours(npaContours)
    ...
    if largestContourInImage is not None:
        ...
        # deskew the image
        roi = ICRecognition.deskewImageBasedOnContour(largestContourInImage, imgGrayAndBlurred)
        ...
        # cv2.imshow("deskewed", roi)
        ret, imgThresh = cv2.threshold(roi, threshVal + ICRecognition.threshToAddForDetail, 255, cv2.THRESH_BINARY)
        ...
        roi = cv2.resize(imgThresh, (ICRecognition.widthImg, ICRecognition.heightImg), interpolation=cv2.INTER_LINEAR)
